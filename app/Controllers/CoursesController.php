    ...
    public function run(){
        ...
        if ($sess -> checkSess()){
            if($sess -> accesible($_SESSION['Email'],'Profile')){
                ...
                //step 3 - set model
                $this -> setModel(new CoursesModel('courses'));

                //step 4 - set template
                $this -> view -> setTemplate(TEMPLATE_DIR . '/courses.tpl.php');
                ...
                //user is valid but and can create session but not allowed to access the pages (Not added to acces array)
